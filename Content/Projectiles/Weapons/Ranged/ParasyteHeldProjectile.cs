...
public class ParasyteHeldProjectile : ModdedFriendlyProjectile
{
    ...
    public override void OnSpawn(IEntitySource source)
    {
        ...
        var unitVectorTowardsMouse =
            mountedCenter.DirectionTo(Main.MouseWorld).SafeNormalize(Vector2.UnitX * player.direction);
    }
    ...
    public override void AI()
    {
        ...
        {
            var holdoutDistance = 32f * Projectile.scale;
        ...
        var dividend = swingTime / (Frames * 2f);
        switch (AITimer1)
        {
        ...
        if (AITimer1 >= dividend * 8.75f && !Throwing)
        {
            ...
            if (player.whoAmI == Main.myPlayer)
                Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), player.Center, Projectile.velocity,
                    (int)AITimer2, Projectile.damage, Projectile.knockBack, Projectile.owner);
            Throwing = true;
        }
        ...
        Projectile.Center =
            playerCenter +
            new Vector2(-0,
                -0); // Centers the projectile on the player. Projectile.velocity will be added to this in later Terraria code causing the projectile to be held away from the player at a set distance.
        Projectile.rotation = Projectile.velocity.ToRotation() + (Projectile.velocity.X < 0f ? -MathHelper.Pi : 0);
        player.itemRotation = (Projectile.velocity * Projectile.direction).ToRotation();


        if (AITimer1 >= swingTime || player.itemAnimation <= 0) Projectile.Kill();
