...
using TerrariaParadox.Content.Projectiles.Weapons;
...
public class WalkingHivePuke : ModdedHostileProjectile
{
    ...
    public void Visuals()
    {
        for (int num1054 = 0; num1054 < 5; num1054++)
        {
            ...
            int num14 = 4;
            int num25 = Dust.NewDust(new Vector2(Projectile.position.X + (float)num14, Projectile.position.Y + (float)num14), Projectile.width - num14 * 2, Projectile.height - num14 * 2, ModContent.DustType<WalkingHivePukeDust>(), 0f, 0f, 100, default, 1.2f);
        ...
        {
            int num36 = 4;
            int num47 = Dust.NewDust(new Vector2(Projectile.position.X + (float)num36, Projectile.position.Y + (float)num36), Projectile.width - num36 * 2, Projectile.height - num36 * 2, ModContent.DustType<WalkingHivePukeDust>(), 0f, 0f, 100, default, 0.6f);
