...
public abstract class ModdedChestTile : ModTile
{
    ...
    public override void SetStaticDefaults()
    {
        ...
        if (CanBeLocked) AddMapEntry(MapColor, this.GetLocalization("MapEntry1"), MapChestName);
        ...
        RegisterItemDrop(ModContent.ItemType<LeechwoodChest>(), 1);
        ...
        if (CanBeLocked) LockedText = this.GetLocalization("Locked");
    }
    ...
    */
    ...
    {
        return CanBeLocked && Main.tile[i, j].TileFrameX / 36 == 1;
    }

    /// <summary>
    ///     Adds a custom condition to unlocking the chest besides having the key in your inventory.
    /// </summary>
    /// <returns>Whether the condition is met. True unlocks the chest.</returns>
    public virtual bool CanUnlockLockedChest()
    {
        return true;
    }
    ...
    public override bool UnlockChest(int i, int j, ref short frameXAdjustment, ref int dustType, ref bool manual)
    {
        ...
        return CanUnlockLockedChest();
    }

    /// <summary>
    ///     Adds a condition for locking the chest while it is unlocked. An example could be placing a lock on the chest, or
    ///     checking whether a boss has been defeated yet.
    /// </summary>
    /// <returns>Whether the condition is met. True locks the chest.</returns>
    public virtual bool CanLockUnlockedChest()
    {
        return true; //Condition.DownedPlantera.IsMet(); for biome chest
    }
    ...
    public override bool LockChest(int i, int j, ref short frameXAdjustment, ref bool manual)
    {
        ...
        if (style == 0) return CanLockUnlockedChest();
        return false;
    }
        ...
        else
        {
            if (isLocked)
            {
                ...
                if (player.HasItemInInventoryOrOpenVoidBag(key) && Chest.Unlock(left, top) &&
                    player.ConsumeItem(key, includeVoidBag: true))
    ...
    public override void MouseOver(int i, int j)
    {
        ...
        {
            var defaultName =
                TileLoader.DefaultContainerName(tile.TileType, tile.TileFrameX,
                    tile.TileFrameY); // This gets the ContainerName text for the currently selected language
            player.cursorItemIconText = Main.chest[chest].name.Length > 0 ? Main.chest[chest].name : defaultName;
            if (player.cursorItemIconText == defaultName)
            {
                player.cursorItemIconID = ChestItemType;
                if (Main.tile[left, top].TileFrameX / 36 == 1) player.cursorItemIconID = KeyItemType;
