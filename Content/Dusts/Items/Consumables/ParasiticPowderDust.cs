...
public class ParasiticPowderDust : ModDust
{
    ...
    public override bool Update(Dust dust)
    {
        ...
        var num105 = dust.scale * 0.8f;
        if (num105 > 1f) num105 = 1f;
        num105 = dust.scale * 0.4f;
        Lighting.AddLight((int)(dust.position.X / 16f), (int)(dust.position.Y / 16f), num105, num105 * 0.5f,
            num105 * 0.3f);
        if (dust.scale < 0.15f) dust.active = false;
        if (!dust.active) return true;
        return false;
    }
}