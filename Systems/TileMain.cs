...
public partial class ParadoxTile : GlobalTile
{
    ...
    {
        if (j >= Main.worldSurface - 1) return; // only grows above ground

        var tile = Main.tile[i, j];
        if (!tile.HasUnactuatedTile) return; // Don't grow on actuated tiles.
        ...
        if ((_flippedVineBlocksList.Contains(tile.TileType) || tile.TileType == _flippedVine) &&
            WorldGen.GrowMoreVines(i, j))
        {
            var growChance = FlippedVine.GrowChance;
            if (tile.TileType == _flippedVine)
                growChance /= 10; // 10 times more likely to extend an existing vine than start a new vine

            var below = j + 1;
            var tileBelow = Main.tile[i, below];
            if (WorldGen.genRand.NextBool(growChance) && !tileBelow.HasTile && tileBelow.LiquidType != LiquidID.Lava)
            {
                ...
                var vineIsHangingOffValidTile = false;
                for (var above = j; above > j - 10; above--)
                {
                    var tileAbove = Main.tile[i, above];
                    if (tileAbove.BottomSlope) return;
                ...
                if (vineIsHangingOffValidTile)
                {
                    ...
                    if (Main.netMode == NetmodeID.Server) NetMessage.SendTileSquare(-1, i, below);
                }
            }
        }
    }
    ...
    public override bool TileFrame(int i, int j, int type, ref bool resetFrame, ref bool noBreak)
    {
        ...
        if (!TileID.Sets.IsVine[type]) return true;
        ...
        var aboveTileType = tileAbove.HasUnactuatedTile && !tileAbove.BottomSlope ? tileAbove.TileType : -1;
        ...
        if (type != aboveTileType)
        {
            ...
            if ((_flippedVineBlocksList.Contains(aboveTileType) || aboveTileType == _flippedVine) &&
                type != _flippedVine)
            {
