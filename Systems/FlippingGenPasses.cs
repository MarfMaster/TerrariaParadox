...
public class FlippingGenPasses : ModSystem
{
    ...
    public override void ModifyWorldGenTasks(List<GenPass> tasks, ref double totalWeight)
    {
        if (WorldBiomeManager.WorldEvilBiome.Type == ModContent.GetInstance<FAltBiomeMain>().Type)
        {
            ...
            var spreadingGrassIndex =
                tasks.FindIndex(spreadingGrassIndex => spreadingGrassIndex.Name.Equals("Spreading Grass"));
            ...
            var cactusPalmIndex =
                tasks.FindIndex(cactusPalmIndex => cactusPalmIndex.Name.Equals("Cactus, Palm Trees, & Coral"));

            var stalacIndex = tasks.FindIndex(stalacIndex => stalacIndex.Name.Equals("Stalac"));

            var finalCleanupIndex =
                tasks.FindIndex(finalCleanupIndex => finalCleanupIndex.Name.Equals("Final Cleanup"));
            ...
            {
                tasks.Insert(cactusPalmIndex + indexFixer,
                    new FlipCactusPalmPass("Flipping Cactus & Palm Trees", 100f));
                indexFixer++;
            }
...
public class FlippingPotsPass : GenPass
{
    ...
    protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
    {
        ...
        var tileType = (ushort)ModContent.TileType<FlippedPots>();

        //+X ist nach rechts

        for (var i = 0; i < FlipsideEvilPass.FlipsideBottomEdges.Count; i++)
        {
            var indexYCoordsMin = FlipsideEvilPass.FlipsideBottomEdges[i] + 40.0 + FlippingGenPasses.ExtraBoundary;
            for (var indexXCoords = FlipsideEvilPass.FlipsideWestEdges[i] - FlippingGenPasses.ExtraBoundary;
                 indexXCoords < FlipsideEvilPass.FlipsideEastEdges[i] + FlippingGenPasses.ExtraBoundary;
                 indexXCoords++)
            {
                indexYCoordsMin += WorldGen.genRand.Next(-2, 3);
                if (indexYCoordsMin < FlipsideEvilPass.FlipsideBottomEdges[i] + 30.0)
                    indexYCoordsMin = FlipsideEvilPass.FlipsideBottomEdges[i] + 30.0;
                if (indexYCoordsMin > FlipsideEvilPass.FlipsideBottomEdges[i] + 50.0)
                    indexYCoordsMin = FlipsideEvilPass.FlipsideBottomEdges[i] + 50.0;
...
public class FlipGrassPass : GenPass
{
    ...
    protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
    {
        ...
        for (var i = 0; i < FlipsideEvilPass.FlipsideBottomEdges.Count; i++)
        {
            ...
            for (var indexXCoords = FlipsideEvilPass.FlipsideWestEdges[i];
                 indexXCoords < FlipsideEvilPass.FlipsideEastEdges[i];
                 indexXCoords++)
            {
                indexYCoordsMin += WorldGen.genRand.Next(-2, 3);
                if (indexYCoordsMin < FlipsideEvilPass.FlipsideBottomEdges[i] + 30.0)
        ...
        for (var i = 0; i < FlipsideEvilPass.FlipsideBottomEdges.Count; i++)
        {
                 ...
                 indexXCoords++)
            {
                ...
                while (indexYCoordsMax < indexYCoordsMin)
                {
                    ...
                    var validTiles = new List<ushort>
                    {
                    ...
                    if (tile.HasTile && validTiles.Contains(tile.TileType) && !aboveTile.HasTile)
                    {
                        ...
                        if (WorldGen.genRand.NextBool(FlippedThorns.GrowChance * 3))
                        {
                            ...
                            if (WorldGen.genRand.NextBool(2))
                                for (var j = 0; j < 40; j++)
                                {
                                    var t = Main.tile[indexXCoords + successX, indexYCoordsMax - successY];
                                    if (!t.HasTile)
                                    {
        ...
        for (var i = 0; i < FlipsideEvilPass.FlipsideBottomEdges.Count; i++)
        {
                 ...
                 indexXCoords++)
            {
                ...
                while (indexYCoordsMax < indexYCoordsMin)
                {
                    ...
                    if (validTiles.Contains(tile.TileType) && WorldGen.genRand.NextBool(FlippedVine.GrowChance / 11) &&
                        !firstbelowTile.HasTile)
                        for (var j = 0; j < WorldGen.genRand.Next(1, 11); j++)
                        {
                            var belowTile = Main.tile[indexXCoords, indexYCoordsMax + j];
                            if (!belowTile.HasTile) belowTile.ResetToType((ushort)ModContent.TileType<FlippedVine>());
                        }
...
public class FlipCactusPalmPass : GenPass
{
    ...
    protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
    {
        ...
        foreach (var p in FlipsideEvilPass.SandToBeFlipped)
        {
            ...
            if (tile.TileType == TileID.Sand) tile.TileType = (ushort)ModContent.TileType<AssecsandBlockTile>();
        }
    }
}

public class
    FlipStalacPass : GenPass //this just doesn't work despite me doing everything the same as in randomupdate, where it works(they do grow naturally yes)
{
                    ...
                    if (indexYCoordsMax > GenVars.worldSurfaceHigh - 1.0)
                    {
                            ...
                            tile.BlockType == BlockType.Solid)
                        {
                            if (!oneAboveTile.HasTile) //check for empty space and whether this block is solid
                            {
                                if (WorldGen.genRand.NextBool(2)) //1 in X chance
                                {
                                    ...
                                    oneAboveTile.TileFrameX =
                                        (short)(frameX * 18); //reframe it so it can show alternate styles
                                }
                                ...
                                {
                                    WorldGen.Place1x2(indexXCoords, indexYCoordsMax - 1,
                                        (ushort)ModContent.TileType<AssecstoneStalagmitesNatural>(), 0);
                                    oneAboveTile.TileFrameX =
                                        (short)(frameX * 18); //need to reframe both tiles to the same frame
                            ...
                            if (!oneBelowTile.HasTile)
                            {
                                ...
                                else if (WorldGen.genRand.NextBool(AssecstoneStalactitesNatural.GrowChance) &&
                                         !twoBelowTile.HasTile)
                                {
                                    WorldGen.Place1x2Top(indexXCoords, indexYCoordsMax + 1,
                                        (ushort)ModContent.TileType<AssecstoneStalactitesNatural>(), 0);
                                    oneBelowTile.TileFrameX = (short)(frameX * 18);
                                 ...
                                 tile.BlockType == BlockType.Solid)
                        {
                            if (!oneBelowTile.HasTile)
                            {
                                ...
                                else if (WorldGen.genRand.NextBool(MurkyIcicles1x2Natural.GrowChance / 4) &&
                                         !twoBelowTile.HasTile)
                                {
                                    WorldGen.Place1x2Top(indexXCoords, indexYCoordsMax + 1,
                                        (ushort)ModContent.TileType<MurkyIcicles1x2Natural>(), 0);
                                    oneBelowTile.TileFrameX = (short)(frameX * 18);
                                    twoBelowTile.TileFrameX = (short)(frameX * 18);
                                }
